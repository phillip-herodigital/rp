...
this.init();}};init();return P;})();﻿require.config({
    baseUrl: "/sitecore/apps/js",
    catchError : false
});
...
(function ($) {
    $(function() {
        $("script").each(function() {
            var context = requirejs.s.contexts["_"],
                src = $(this).attr('src') || '',
                shortname = src.replace(requirejs.s.contexts["_"].config.baseUrl, "").replace(".js", "");

            context.urlFetched[src] = true;
            context.loaded[shortname] = true;
            context.defined[shortname] = null;
        });   
    });
})(jQuery);﻿(function ($) {
    $.extend(true, $, {
        sc: {
            location: function (params, activeUrl) {
                var cUrl = activeUrl,
                    urlParams = null;
                if (cUrl == null || typeof cUrl != 'string') {
                    cUrl = $(activeUrl).closest('form').attr('action') || location.href;
                    cUrl = (cUrl || window.location.pathname);
                }
                var rawUrl = cUrl;
                if (cUrl.indexOf("?") > -1) {
                    urlParams = cUrl.substring(cUrl.indexOf("?") + 1, cUrl.length).split('&');
                ...
                return { url: cUrl, raw : rawUrl, search: $.extend(false, data, params) };
            }
        }
    });
})(jQuery);﻿(function ($) {
    $.extend($, {
        Storage: function (options) {
            this.options = {
                expires: 7,
                domain: window.location.hostname + '_storage'
            };
            options ? (typeof (options) == 'string' ? this.options.name = options : this.options = $.extend({}, this.options, options)) : $.noop();
            var storageKey;
            if (this.options.storageKey) {
                storageKey = this.options.storageKey;
                delete this.options.storageKey;
            } else {
                storageKey = document.location.href.replace(/[\/,\?,\&,\-,\{,\},\=\#\%\.\:]/g, '');
            }
            var store = new Persist.Store(storageKey, this.options);
            //var self = this;
            this.get = function(name) {
                var retVal = store.get(name);
