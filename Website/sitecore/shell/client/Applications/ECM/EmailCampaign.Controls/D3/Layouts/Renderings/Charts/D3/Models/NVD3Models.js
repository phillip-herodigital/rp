    ...
    */
    ellipse: function(d3Element, width, paddingLeft, paddingRight, startEllipse, addTitle) {
      ...
      var
        ...
        originalText = text,
        availableSpace = (width - paddingRight - paddingLeft);
      if (text.length === 0) {
        return text;
      }
    ...
    models.nvd3Legend = {
        ...
        */
        ...
        createLegend: function (chartProperties) {
            ...
            d3.select("#svg-legend-" + chartProperties.componentElement.attr(models.constants.idSelector)).style("height", models.constants.legendItemHeight + "px");
        ...
        createVerticalLegend: function (chartProperties, chartHeight) {
            ...
            chartProperties.chartData.forEach(function (item) {
              ...
              background.on('click', function () {
                    ...
                    if (chartProperties.chartData[seriesIndex].legendState === models.constants.legendState.checked && 
                        chartProperties.chartType === models.constants.chartType.doughnut) {
                       
                        models.nvd3VisualizationManager.visualizeInsideLineElements(
                              chartProperties.svgElement,
                              models.constants.visualizationMode.standard,
                              chartProperties.selectedIndex,
                              chartProperties.palette,
                              chartProperties.sizes);
                    }
      ...
      if (chartType === models.constants.chartType.line || chartType === models.constants.chartType.area) {
        ...
        chartElement.interactiveLayer.tooltip.contentGenerator(function (d) {
          ...
          for (seriesIndex = 0; seriesIndex < dataLength; seriesIndex++) {
            ...
            switch (chartType) {
              case models.constants.chartType.line:
                if (data[index].values.length >= d.index) {
...
(function (models) {
  models.nvd3Axis = {
    ...
    */
    removeOverlappingLabels: function(canvas, hasBarInCombinationChart) {      
      var margin = 10;
      var ticks = canvas.selectAll(".nv-x.nv-axis.nvd3-svg .tick");

      var drawingCanvasRight = canvas.select(".nv-x.nv-axis.nvd3-svg").node().getBoundingClientRect().right;
      var startIndex = hasBarInCombinationChart ? 1 : 0;
      var endIndex = hasBarInCombinationChart ? ticks[0].length - 1 : ticks[0].length;
      for (var j = startIndex; j < endIndex; j++) {
        var c = ticks[0][j],
          n = ticks[0][j + 1];
          if (c && c.getBoundingClientRect && c.getBoundingClientRect().right > drawingCanvasRight) {
            d3.select(c).remove();
            continue;
          }
    ...
    */
    createXaxis: function (canvas, element, margin, showXAxis, hasBarInCombinationChart, labelTrimmingLocation) {      
      ...
      setTimeout(function () {
        if (hasBarInCombinationChart) {
          this.markFirstAndLastLabel(canvas);
          this.removeXaxisMinMax(canvas);
        }
        
        this.removeOverlappingLabels(canvas, hasBarInCombinationChart);
        if (showXAxis) {
    ...
    * Creates the xAxis.
    * @param {object} canvas - The canvas element.
    * @param {int} numberOfTicks - The number of ticks in the axis.
    */
    markFirstAndLastLabel: function(canvas) {
      var xAxisLabels = canvas.selectAll("g.nv-focus > g.nv-x.nv-axis.nvd3-svg g.tick text");
      d3.select(xAxisLabels[0][0]).classed("firstLabel", true);
      d3.select(xAxisLabels[0][xAxisLabels[0].length - 1]).classed("lastLabel", true);
    },
    ...
    * @param {int} numberOfTicks - The number of ticks in the axis.          
    */
    removeXaxisMinMax: function(canvas) {
      var firstLabel = canvas.select("g.nv-focus > g.nv-x.nv-axis.nvd3-svg g.tick text.firstLabel");
      d3.select(firstLabel[0][0]).classed("hide", true);
      var lastLabel = canvas.select("g.nv-focus > g.nv-x.nv-axis.nvd3-svg g.tick text.lastLabel");
      d3.select(lastLabel[0][0]).classed("hide", true);
    },
