...
namespace StreamEnergy.RenderingService
{
    class Program
    {
        static void Main()
        {
            ...
            var rasterizer = new Rasterizer(dir, new Uri("http://localhost:59203/"));
            var redisDb = multiplexer.GetDatabase();
            var listener = new RedisQueueListener(redisDb, "EnrollmentScreenshots");
            ...
            MainLoop(listener, rasterizer, dir).Wait();
        }

        private static async Task MainLoop(RedisQueueListener listener, Rasterizer rasterizer, string dir)
        {
            await Task.Yield();
            while (true)
            {
