...
namespace StreamEnergy.DomainModels.Enrollments
{
    public class CompleteOrderState : StateBase<UserContext, InternalContext>
    {
        ...
        {            
            ...
            if (!data.IsRenewal && !data.IsAddLine && !data.IsSinglePage)
            {
            ...
            if (!data.IsRenewal && !data.IsAddLine && !data.IsSinglePage && !data.Services.SelectMany(s => s.Location.Capabilities).OfType<CustomerTypeCapability>().Any(ct => ct.CustomerType == EnrollmentCustomerType.Commercial))
            {
