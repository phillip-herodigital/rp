...
using DCElectricity = StreamEnergy.DomainModels.Enrollments.DCElectricity;
using StreamEnergy.DomainModels.Accounts;


namespace StreamEnergy.Services.Clients
{
    class DCElectricityAdapter : ILocationAdapter
    {
        private readonly ISitecoreProductData sitecoreProductData;

        public DCElectricityAdapter(ISitecoreProductData sitecoreProductData)
        {
        ...
        {
            return location.Address.StateAbbreviation == "DC" || location.Capabilities.OfType<DCElectricity.ServiceCapability>().Any();
        }

        bool ILocationAdapter.IsFor(IEnumerable<IServiceCapability> capabilities, IOffer offer)
        {
            return offer.OfferType == DCElectricity.Offer.Qualifier;
        }

        bool ILocationAdapter.IsFor(Address serviceAddress, string productType)
        {
            return serviceAddress.StateAbbreviation == "DC" && productType == "Electricity";
        }

        bool ILocationAdapter.IsFor(DomainModels.Accounts.ISubAccount subAccount)
        {
            return subAccount is DomainModels.Accounts.DCElectricityAccount;
        }
        ...
        {
            var capability = capabilities.OfType<DCElectricity.ServiceCapability>().Single();
        ...
        DomainModels.Enrollments.LocationOfferSet ILocationAdapter.LoadOffers(DomainModels.Enrollments.Location location, StreamConnect.ProductResponse streamConnectProductResponse)
        {
            ...
            {
                return LoadDCOffers(location, streamConnectProductResponse);
            }
            else
            {
                ...
                {
                    Offers = new[] {
                            new DCElectricity.CommercialQuote { }
                    }
                };
            }
        }

        private LocationOfferSet LoadDCOffers(Location location, StreamConnect.ProductResponse streamConnectProductResponse)
        {
            ...
            return new LocationOfferSet
            {
                Offers = (from product in streamConnectProductResponse.Products
                          ...
                          let productData = sitecoreProductData.GetDCElectricityProductData(product.ProductCode.ToString())
                          where productData != null
                          select new DCElectricity.Offer
                          {
        ...
        dynamic ILocationAdapter.ToEnrollmentAccount(Guid globalCustomerId, EnrollmentAccountDetails account, bool IsAutoPayEnabled, string ExistinDCccountNumber, DateTime DOB, string Gender)
        {
            var DCElectricityOffer = account.Offer.Offer as DCElectricity.Offer;
            var DCElectricityService = account.Location.Capabilities.OfType<DCElectricity.ServiceCapability>().Single();
            ...
            return new
            {
                ...
                Premise = new
                {
                    ...
                    UtilityProvider = JObject.Parse(DCElectricityOffer.Provider),
                    UtilityAccountNumber = (account.Offer.OfferOption is DCElectricity.SwitchOfferOption) ? ((DCElectricity.SwitchOfferOption)account.Offer.OfferOption).PreviousAccountNumber : DCElectricityService.PreviousAccountNumber,
                    Product = JObject.Parse(DCElectricityOffer.Product),
        ...
        {
            if (offer is DCElectricity.Offer)
            {
                return JObject.Parse(((DCElectricity.Offer)offer).Provider);
            }
            return null;
        }
        ...
        {
            var account = subAccount as DomainModels.Accounts.DCElectricityAccount;
        ...
        {
            var result = new DomainModels.Accounts.DCElectricityAccount
            {
            ...
            {
                var productData = sitecoreProductData.GetDCElectricityProductData((string)details.Product.ProductCode) ?? new SitecoreProductInfo
                {
        ...
        {
            return new StreamEnergy.DomainModels.Enrollments.DCElectricity.RenewalCapability { Account = account, SubAccount = subAccount };
        }
        ...
        {
            var DCElectricityOffer = offer as DCElectricity.Offer;
            code = DCElectricityOffer.Code;
            id = DCElectricityOffer.Id.Split(new[] { '/' }, 2)[1];
        }
    }
}
